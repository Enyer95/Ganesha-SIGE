...
////////////////////////////////////////////////////////////////////////////7777
    public function qrGenerador(Request $request){
        QrCode::format('png')->size(399)->color(76, 175, 80)->merge('/public/img/logoss.png', .2)->errorCorrection('H')->generate(Crypt::encrypt($request->text), '../public/qr_image/Texto.png');  

        return redirect('/home')->with(['tipoMsj'=>'success','msj'=> 'Busquelo en la carpeta','titulo'=> 'QR Generado']);
    }
    
    public function ScanTest(){
        $qrcode = new \QrReader('../public/qr_image/Texto.png');
        $text = $qrcode->text(); //return decoded text from QR Code
        return redirect('/home')->with(['tipoMsj'=>'success','msj'=> Crypt::decrypt($text),'titulo'=> 'QR Generado']);
    }
////////////////////////////////////////////////////////////////////////////////
    ...
    //Creo una variable la cual Me trae toda l
    //a informacion que contiene la base de datos


    return view('Usuario/G_usu')->with(['usuarios' => $usuarios]);
    //Y Retorno a la vista una
    //variable a la cual le asigno lo que contiene la variable anterior creada
    }
    ...
    public function agregar(Request $request)
    {
        $this->validate($request, [
            ...
            if($uservalid>0){
                ...
                try{
                 $usuario-> private_pass = $passPrivate;

                    $usuario->generatenotify((new InvoicePaid($usuario)));
                } catch (\Exception $e) {
                         // dd('error');
                }
                return redirect('Usuario/G_usu');
