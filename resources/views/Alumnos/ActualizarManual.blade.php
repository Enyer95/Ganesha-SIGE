...
<form role="form" method="POST" action="{{ url('/modificar_alumnos_manual') }}" id="formulario">
{{--  --}}
	{{ csrf_field() }}
				        ...
				        <tr id="tr{{ $alum->ci_est}}" class="cd{{$i}}">
							...
							<td> 
								<button type='button' id="{{ $alum->ci_est}}" value="{{ $busq->id_sec_est}}" onclick="eliminar(this)" class="btn btn-danger btn-xs"> 
                                    <img src="{{ url('/img/iconos/16x16/cancel.png') }}">
                                </button> 
                            </td>			            
                        </tr>
