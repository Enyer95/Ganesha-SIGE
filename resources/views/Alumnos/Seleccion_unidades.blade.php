  ...
  <div class="table-responsive">
    ...
    <thead>
      ...
      @foreach($master as $ma)
        <form  method="POST"  action="{{ url('/controlleralumnos/'.$ma->id_uc_sec.'/verifica') }}" >
                        {{ csrf_field() }}
        <tbody>
          ...
          <td>
              <button type='submit' name="Tipo" value="Listado" class='btn btn-primary btn-xs'>
                <i><img src="{{ url('/img/iconos/16x16/book.png') }}"></i>
              </button>
          </td>
          <td>
              <button type='submit' name="Tipo" value="Manual" class='btn btn-xs'>
                <i><img src="{{ url('/img/iconos/16x16/file-add.png') }}"></i>
              </button>
          </td>
        </tbody>
              </form>
