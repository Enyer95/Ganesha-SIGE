...
@foreach($planeva2 as $id_plan)
    <div class="col-md-12">
        ...
        <button type='button' class='btn btn-primary btn-xs'>
          <img src="{{ url('/img/iconos/32x32/edit-alt-1.png') }}" data-toggle='modal' data-target='#ModificarPlanMaestro'>
        </button>               
        <a href="{{ url('/Plan_Evaluaciones/'.$id_plan.'/Asignar') }}" id="Asignar"  onclick="verificoAsig(this)">
            <button type='button' class='btn btn-warning btn-xs'>
              <img src="{{ url('/img/iconos/32x32/elevator.png') }}">
            </button>                       
        </a>
        <a href="{{ url('/controllerplanevaluaciones/'.$id_plan.'/eliminar') }}" id="eliminar" onclick="verifico(this)">
            <button type='button' class='btn btn-danger btn-xs'>
                <img src="{{ url('/img/iconos/32x32/cancel.png') }}">
            </button>                       
        </a>
    </div>
@endforeach
